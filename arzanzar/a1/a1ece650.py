...
def printVertices(outputVertices):

    print("V = {")

    outputVerticesList = list(outputVertices)
    if len(outputVerticesList)==0:
        print("}")
        return
    else:
        for i in range(len(outputVerticesList)):
            print("  "+ str(i+1) + ":   ("+ "{:.2f}".format(outputVerticesList[i][0],2) +","+ "{:.2f}".format(outputVerticesList[i][1],2) +")")
            #print("  "+ str(i+1) + ":   ("+ str(round(outputVerticesList[i][0],2)) +","+ str(round(outputVerticesList[i][1],2)) +")")
        print("}")


def printEdges(outputVertices,segmentAndIntersectList):
    ...
    for t in range(len(edgeOrder)):
        if t+1 == len(edgeOrder):
            ...
            endprint = '>,'    
        print("  <"+str(edgeOrder[t][0])+","+str(edgeOrder[t][1])+endprint)
...
def parseGenerate(commandValue):
    ...
    printEdges(outputVertices,segmentAndIntersectList)
